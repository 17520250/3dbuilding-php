      ...
      ?>
       ...
       const geojsonLayer<?= $row->MaKienTruc ?> = new GeoJSONLayer({
         ...
         popupTemplate: {
           ...
           content: "Loại kiến trúc: <?= $row->TenLoaiKienTruc ?><br>Vật liệu: <?= $row->TenVatLieu ?><br>Tầng: <?= $row->TenTang ?><br>Ngày hoàn thành: <?= $row->NgayHoanThanh ?><br>Hạn sử dụng vật liệu: <?= $row->HanSuDungVL ?> tháng<br>Hạn bảo trì: <?= $row->HanBaoTri ?><br>Trạng thái bảo trì: <?php if ($row->TrangThaiBT === '#00ff00') {echo 'Còn xa hạn bảo trì, còn <b>' . $row->SoNgayDenBT . '</b> ngày';} else {echo 'Sắp đến hạn bảo trì, còn <b>' . $row->SoNgayDenBT . '</b> ngày';} ?>"
         },
       });
